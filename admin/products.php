...
if ($_POST){
  ...
  if ($photoCount > 0){
    ...
    $dbpath .='/choiceshop/images/products/'.$uploadName;
     if ($mimeType != 'image'){
  ...
  if($photoCount > 0){
    ...
    if(isset($_GET['edit'])){
      ...
      `list_price` ='$list_price',`brand` ='$brand',`categories`='$category',`sizes`='$sizes',`image`='$dbpath',`description`='$description'
       WHERE id ='$edit_id'";
    }
    $db->query($insertSql);
    header('Location: products.php');
